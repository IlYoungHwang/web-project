		...
		<div class="jumbotron">
			<div class="row">
				...
				<div class="col-sm-3 col-md-3">
 					<div class="thumbnail">
						<img src="/displayFile?fileName=${usedVO.fullName}" alt="이미지" width="300" height="300">
						<div class="caption">
							<h3>${usedVO.name}</h3>
							<p>차종 ${usedVO.name}</p>
							<p>사고유무 ${usedVO.acci}</p>
							<p>가격 ${usedVO.pay}만원</p>
							<p><a href="/carview/usedInfo${pageMaker.makeSearch(pageMaker.cri.page)}&no=${usedVO.no}&userno=${usedVO.userno}" class="btn btn-primary" role="button">예약하기</a></p>
						</div>
					</div>
 				</div>   
 				</c:forEach>
		...
		<div class="text-center">
						<ul class="pagination">
							...
							<c:if test="${pageMaker.prev}">
								<li><a href="usedList${pageMaker.makeSearch(pageMaker.startPage - 1)}">&laquo;</a></li>
							</c:if>
							...
							<c:forEach begin="${pageMaker.startPage }"
								...
								<li
									...
									<a href="usedList${pageMaker.makeSearch(idx)}">${idx}</a> 
								</li>
							</c:forEach>

							<c:if test="${pageMaker.next && pageMaker.endPage > 0}">
								<li><a href="usedList${pageMaker.makeSearch(pageMaker.endPage + 1)}">&raquo;</a></li>
							</c:if>
						</ul>
	</div>
